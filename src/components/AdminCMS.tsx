...
export const AdminCMS: React.FC<AdminCMSProps> = ({ onNavigate }) => {
  ...
  // Publish content to GitHub via Cloudflare Worker
  const handlePublishToGitHub = async () => {
    if (!workerUrl) {
      setIsSettingsOpen(true);
      showToast('請先在設定中輸入 Cloudflare Worker 代理網址', 'error');
      return;
    }
    ...
    try {
      ...
      showToast('🎉 發佈成功！GitHub content.json 已更新，前台即時動態同步！', 'success');
    } catch (err: any) {
      showToast(`GitHub 雲端發佈失敗：${err.message}`, 'error');
    } finally {
            ...
            <div className="space-y-4 text-xs max-h-[70vh] overflow-y-auto pr-1">
              ...
              <div className="p-3.5 bg-amber-950/20 border border-amber-500/30 rounded-xl space-y-2.5">
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  本網站已採用「程式碼與資料徹底分離」架構，專案中無任何靜態資料檔案。點擊下方按鈕可強制從 GitHub 重新拉取最新照片與作品資料。
                </p>
                <div className="pt-1">
                  <button
                    type="button"
                    onClick={async () => {
                      ...
                      showToast('✅ 成功同步最新遠端內容！', 'success');
                    }}
                    className="w-full px-3 py-2 bg-amber-600/90 hover:bg-amber-500 text-slate-950 font-bold rounded-lg text-xs flex items-center justify-center gap-1.5 transition-all shadow-sm"
                  >
                    <DownloadCloud className="w-3.5 h-3.5" />
                    <span>立即從 GitHub 雲端重新整理</span>
                  </button>
                </div>
              </div>
